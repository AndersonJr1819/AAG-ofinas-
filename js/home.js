document.addEventListener('DOMContentLoaded', () => {
    ...
    if (container) {
        ...
        const body = new THREE.Mesh(
            new THREE.BoxGeometry(3, 1.2, 1.4),
            new THREE.MeshStandardMaterial({ color: 0x181818, roughness: 0.4, metalness: 0.6 })
        );
        body.position.y = 0.6;
        group.add(body);

        const cabin = new THREE.Mesh(
            new THREE.BoxGeometry(1.2, 1, 1.35),
            new THREE.MeshStandardMaterial({ color: 0xff6b00, roughness: 0.3 })
        );
        ...
        loader.load(
            ...
            undefined,
            (error) => {
                console.warn('Erro ao carregar o modelo GLTF:', error);
            }
        );
